...
**M7 — Audit-superseded block-table figures.** The [SPOTCHECK.md](https://github.com/sanskrit-lexicon/MWS/blob/master/papers/microanalysis/analysis/SPOTCHECK.md) audit supersedes the early estimates the table used: F03 5 %→3.3 %, F05 <1 %→2.5 %, F16 9 %→6.5 %, F08 21 %→23.2 % (with the ~⅓ compound false-positive caveat), F06 6 %→7.1 %, F04 65 %→67.5 %, F13 →13.9 %; and "lex-hedged = 40 212 записей (14 %)" conflated tags with entries — entries are **38,414 (13.4 %)**, tags 40,212. *Fix (applied):* audited values + a table provenance note + the entries/tags distinction.
