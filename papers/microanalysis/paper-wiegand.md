...
Each top-level lemma (`<e>1`) with derivable compounds carries a tail of `<e>3*` compound sub-entries clustered in print sequence under its parent. *aṃśu* (L47) is followed in mw.txt by 9 continuation senses (L48–56) and then ~40 compound sub-entries (L57+). The cluster has explicit hierarchical integration through the `<e>3` indicator + the em-dash `—` in `<k2>`.
