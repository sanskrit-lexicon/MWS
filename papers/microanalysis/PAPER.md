...
This is **block economy** in the explicit sense: **a 19th-century printed dictionary cannot afford to elaborate every block in every entry**. Print space is finite; setting cost is real; the user must be able to scan. MW's design rationalises this by maintaining a 6-block kernel and adding to it only when the article-type demands. MW's source dictionary PWG is far denser: it carries [more `<ls>` citations in total](../../DICT_PROFILE.md#lineage-wil--koshas-mw--pwg) (570,817 vs MW's 311,932) across **less than half** the entries, so **per entry PWG is ~4× more citation-dense** (4.6 vs 1.1 `<ls>` per record; [cross-dict audit](analysis/CROSS_DICT.md)). PWG can afford this because it is a multi-volume work; MW1899 is one volume, and its lower density is the print-economic constraint made visible. Importantly, the cross-dict audit ([§9.3](#9-methodological-limitations)) shows the *economy shape itself* — a small modal kernel plus a long tail — is **common to all single-volume CDSL dictionaries, not unique to MW**; we therefore present block economy as the property *characteristic of* single-volume scholarly dictionaries that MW instantiates, rather than as an MW invention.
...
**(ii) The `<ls>L.</ls>` hedge is MW's most distinctive *systematisation* — though not its semantic invention.** The grounded reading names it the *hedge construct* (Construct 4). Wiegand identifies it as a transverse *commenting structural indicator* and a deliberate *Strukturveränderung* of PWG's source-attribution system (Appendix A §A.3). Atkins & Rundell classify it as a *pragmatic register marker* of the koshic-only register (Appendix B §B.5). Hausmann's framework has *no slot for it* and must be extended with a proposed fifth comment-class, the *Provenienz-Komment* (Appendix C §C.2). All four frameworks agree it is specific to MW in the CDSL corpus (40,212 instances vs 0 in PWG, PWK, WIL, BEN, CAE, SKD, VCP and 1 in AP), absent from PWG's named-kosha apparatus, and a deliberate editorial compression of PWG's named-kosha citation system. The semantic precedent, however, is older: Cappeller 1891 asterisk `*` ("word taught only by grammarians or lexicographers") and Benfey 1866 dagger `†` ("no authoritative references") used the same conceptual marker as typographic devices (Appendix C §C.2, [analysis/LS_HEDGE_CHECK.md](analysis/LS_HEDGE_CHECK.md#print-preface-read-added-2026-05-27-closes-the-digital-only-gap)). MW's *structural* innovation is to promote the hedge from a marginal typographic mark to a tagged citation occupying the same source slot as `<ls>MBh.</ls>` or `<ls>RV.</ls>`. See [DICT_PROFILE Lineage](../../DICT_PROFILE.md#lineage-wil--koshas-mw--pwg).
...
We flag five limits. Four now have a first round of empirical results from the reproducible audit scripts in [`analysis/`](analysis/) (which reuse the published detector verbatim); each is summarised below with its remaining gap.
...
2. **Statistical significance** — *tested, [analysis/SIGNIFICANCE.md](analysis/SIGNIFICANCE.md)*. Every headline contrast is significant at α = 0.05 (chi-square / Fisher, with Wilson 95% CIs): F09-in-roots 78.1% [75.0, 80.9], F05 98.4%, F07 100%, F13 botanical 71.5% [70.5, 72.4], biographical 64.7% [59.6, 69.6] (wide — N = 346), lexicographer-only 100%, F02-in-continuation 33.9%. The D7 example is **confirmed non-significant**: noun_m F08 (21.6%) vs noun_f F08 (22.6%) differ by only 1.0 pt, *p* = 0.07 — so gender-level F08 differences should not be reported as findings. A full FDR-corrected table ([analysis/SIGNIFICANCE_FULL.md](analysis/SIGNIFICANCE_FULL.md)) confirms **217 of 270** block × type cells remain significant under Benjamini–Hochberg (q = 0.05) — the headline effects are not multiple-comparison artefacts.
...
4. **The `L.`-hedge "MW innovation" claim** — *strengthened, [analysis/LS_HEDGE_CHECK.md](analysis/LS_HEDGE_CHECK.md)*. The generic `<ls>L.</ls>` hedge appears 40,212× in MW (140 per 1k records) and **zero times in PWG (of 570,817 `<ls>` tags), PWK, WIL, CAE, SKD, and VCP; once in AP**. New: **Benfey 1866** — an earlier English-Sanskrit dictionary — has a full citation apparatus (14,708 `<ls>` tags) but **no generic hedge**, which strengthens rather than undermines the innovation claim. **Cappeller 1891 (CAE)** — now fetched, and significant because Cappeller co-edited MW's 1899 edition — likewise has *zero* `<ls>` apparatus, but it carries two **undocumented markers**: an asterisk `*` (1,370×, ~166 headword-initial, the rest entry-final) and a dagger `†` (903×). An entry-final `*` sits exactly where a hedge sits. We checked every available CDSL source for their meaning — `cae_front.txt` (an empty placeholder), `cae-meta2.txt` (its character table omits `*`), and the CAE repo's `DATA_DICTIONARY.md` — and **none documents them**. So fetching CAE *narrows* the gap (no `<ls>` `L.`-apparatus) but cannot close it: the asterisk must be read from Cappeller's **1891 print preface**, not any digital source. The remaining D2 gap is therefore a human read of the **Cappeller / WIL / Benfey print prefaces** for a `*`- or "(L.)"-style convention that a source-tag count cannot detect.
5. **The 14-type typology mixes primary types with orthogonal properties** — *not yet revised*. `noun_m/f/n/mn` differ only in gender (one type, four sub-types); `vedic_accented` and `lexicographer_only` are orthogonal properties (a noun can be both `noun_m` and `vedic_accented`). A more parsimonious version would distinguish ~8 primary article types from orthogonal binary properties (Vedic-accented, lexicographer-hedged, IE-cognate-bearing).
...
**The convention is not, however, an MW *invention*.** A 2026 preface-read of the three competing English-Sanskrit dictionaries available to MW shows the hedge was already in print, twice ([analysis/LS_HEDGE_CHECK.md](analysis/LS_HEDGE_CHECK.md#print-preface-read-added-2026-05-27-closes-the-digital-only-gap)):

- **Cappeller 1891** explicitly defines asterisk `*` as "a word taught only by grammarians or lexicographers" — *semantically the exact analogue* of MW's `<ls>L.</ls>` — and dagger `†` for Prakrit-translation-only words. Cappeller co-edited MW 1899, making the lineage direct.
- **Benfey 1866** defines dagger `†` as "verbs or meanings for which there are no authoritative references" — a weaker methodological hedge ("no source") rather than MW's stronger evidential hedge ("kosha-only"), but the *type* of intervention (an inline marker for under-attested entries) is fully present 33 years before MW.
- **Wilson 1832** appears to lack a systematic inline hedge convention in both its digital record (224 of 230 `<ls>` tags are `<ls>Rox.</ls>`) and in the secondary literature; its source apparatus is base-citation-oriented rather than evidential-class-oriented.
