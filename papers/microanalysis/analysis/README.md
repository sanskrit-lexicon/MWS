...
| [`gold_sample.py`](gold_sample.py) + [`gold_score.py`](gold_score.py) | [GOLD_STANDARD.md](GOLD_STANDARD.md), `GOLD_SAMPLE.json/.md`, `GOLD_STANDARD_SCORES.md` | [D6](../DOUBTS.md#d6--block-detection-is-regex-based-and-approximate--important) | Upgrades the D6 audit to a **200-entry stratified, double-annotated gold standard**: per-block precision/recall/F1 against the adjudicated gold + Cohen's κ between two annotators. `gold_score.py --selftest` checks the metric maths. |
...
| [`ls_hedge_check.py`](ls_hedge_check.py) | [LS_HEDGE_CHECK.md](LS_HEDGE_CHECK.md) | [D2](../DOUBTS.md#d2--the-lslls-claim-refined-mw-systematised-a-convention-pioneered-typographically--resolved-2026-05-27) | Counts the generic `<ls>L.</ls>` hedge and the `<ls>` apparatus across all eight dictionaries. |
...
| [`export_dict_blocks.py`](../figures/scripts/export_dict_blocks.py) | `{code}_blocks.json` files | [Decision 5](../decisions/MICROSITE.md) | Generates per-dict block-matrix JSON exports for all 9 CDSL dicts (input: mw.txt, pwg.txt, etc. from `/tmp`). Includes LEXMAP for cross-dict adjective normalisation. |
...
| [`check_figure_consistency.py`](check_figure_consistency.py) | (prints to stdout) | [Decision 16](../decisions/SUPPLEMENTARY.md#decision-16--supplementary-materials-self-contained-zip) | 28 consistency checks across figure data files and per-dict blocks.json; validates total records, per-type counts, cross-dict MW consistency, per-dict block records. |
