...
| [`cross_dict_profiles.py`](cross_dict_profiles.py) | [CROSS_DICT_PROFILES.md](CROSS_DICT_PROFILES.md) | [D1](../DOUBTS.md#d1--is-block-economy-a-genuine-principle-or-print-economic-artifact--important) | Per-type block *profiles* across the English-format dicts (does type-differentiation generalise beyond MW?). |
...
- **D1** — block-economy *shape* is **general to all single-volume CDSL dictionaries**, not MW-specific (modal blocks/entry: MW 5, PWG 4, PWK 3, AP 2, WIL 3, Benfey 3, Cappeller 3); PWG is ~4× denser per entry. The *profile* construct also generalises (MW type-spread 11.3 pts, AP 15.2). Claims softened in PAPER.md §4/§8/§9.3.
