...
Where MW marks lexicographer-only material with an *exceptional* hedge
(`<ls>L.</ls>`, 13% of citations), the indigenous register cites lexicons by
**default**. The `L.` hedge is Register A's way of flagging, as exceptional, what
is the constitutive norm of Register B. (VCP's `iti`-source vocabulary differs
from SKD's; the SKD-derived kośa list does not measure it — VCP's lexical share
is left open, not zero.)

## 2. Is that lexical vocabulary corpus-grounded?
Yes — about half of it is. SKD headwords are cited in nominative-singular form
(`aMSaH`, `aMSakaM`), so a stem-aware match is required; the de-inflected figure
is the real one (the same de-inflection leaves bare-stem VCP unchanged, a +0.0-pt
control).
