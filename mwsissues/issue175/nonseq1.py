...
def check_groups_5(groups,entries):
 # for each group, check that the associated entries all have
 # no <h>[a-z] in metaline
 nprob = 0
 for group in groups:
  groupkey = group.key
  # groupkey = L1,X1;L2,X2; ...
  ...
  for i,ientry in enumerate(ientries):
   ...
   #if re.search(r'<h>[a-z]',metaline):
   if re.search(r'<h>[^0-9<]+<',metaline):
    flag = False
  group.seqflag5 = flag
  if not flag:
   nprob = nprob + 1
 print('check_groups_5 finds %s problems' % nprob)
 return nprob

def check_groups_6(groups,entries):
 # for each group, check that the associated metalines
 ...
 for group in groups:
  ...
  if not flag:
   nprob = nprob + 1
 print('check_groups_6 finds %s problems' % nprob)
 return nprob
...
def mark_problem_entries_5(groups,entries):
 for entry in entries:
  entry.group = '' # new attribute
 icase = 0
 for group in groups:
...
def mark_problem_entries_6(groups,entries):
 for entry in entries:
  entry.group = '' # new attribute
 icase = 0
 for group in groups:
  if group.seqflag6:  # only difference from mark_problem_entries_5
   continue
  # group entries have different bodies 
  icase1 = icase + 1
  icase = icase1
...
if __name__=="__main__":
 ...
 if option == '2':
  ...
  write_entries1(fileout,entries)
  exit(0)
 nprob = check_groups_5(groups,entries)
 if option == '3':
  # mark groups that have a <h>[a-z] in a metaline
  mark_problem_entries_5(groups,entries)
  write_entries1(fileout,entries)
  exit(0)
 nprob = check_groups_6(groups,entries)
 if option == '4':
  # mark groups that have differences in <e> in metalines
  mark_problem_entries_6(groups,entries)
  write_entries1(fileout,entries)
  exit(0)
