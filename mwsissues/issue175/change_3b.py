...
class Group:
 def __init__(self,key):
  ...
  self.seqflag1 = None # Are datalines same in groups?
...
def entries_update_3b(ientries,entries):
 es = [entries[ientry] for ientry in ientries]
 for e in es:
  # new attribute for each entry in 'es' array
  e.lexes = []
  for line in e.datalines:
   lexes = re.findall(r'<info lex=".*?"/>',line)
   for lex in lexes:
    e.lexes.append(lex)
 # Is there difference among the e.lexes?
 flag = True  # all the e.lexes are the same
 e0 = es[0] # first group entry
 for e in es:
  if e.lexes != e0.lexes:
   flag = False  # a different e.lexes
 if flag:
  return flag
 # change each e by removing the lexes
 for e in es:
  bodiesold = e.datalines
  bodiesnew = []
  for line in bodiesold:
   newline = re.sub(r'<info lex=".*?"/>', '', line)
   bodiesnew.append(newline)
  e.datalines = bodiesnew # revises b
 return flag

def change_3b(groups,entries):
 for entry in entries:
  entry.group = '' # new attribute
 icase = 0
 for group in groups:
  ...
  # group entries have different bodies 
  ientries = group.ientries
  flag = entries_update_3b(ientries,entries)
  if flag:
   continue  # no changes
  icase1 = icase + 1
  icase = icase1
  ...
  entry2 = entries[ientry2]
  entry2.group = '; END case %03d: %s'  % (icase1,group.key)
 print('%s cases altered by change_3b' % icase1)
if __name__=="__main__":
 filein = sys.argv[1] #  xxx.txt (path to digitization of xxx)
 fileout = sys.argv[2] # xxx.txt with some entry changes
 ...
 if nprob != 0:
  print('check_groups_3 found %s problems' % nprob)
  print('Correct these in %s, then rerun' % filein)
  exit(1)
 nprob = check_groups_4(groups,entries)
 #print('check_groups_4 found %s problems' % nprob)
 
 change_3b(groups,entries)
