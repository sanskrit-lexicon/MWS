...
def entries_update_3d(ientries,entries):
 ...
 for e in es:
  ...
  if m == None:
   newmetaline = metaline
  else: # metaline has a homonym 
   h = m.group(1)
   if re.search(r'^[0-9]+$',h):
    newmetaline = metaline # standard homonym code no change
   else:
    # remove the non-standard homonym
    newmetaline = metaline.replace('<h>'+h,'')
  if newmetaline != metaline:
   e.metaline = newmetaline  # revise entry
   flag = False 
 return flag

def change_3d(groups,entries):
 for entry in entries:
  entry.group = '' # new attribute
 icase = 0
 for group in groups:
  ientries = group.ientries
  flag = entries_update_3d(ientries,entries)
  if flag:
   continue  # no changes
  icase1 = icase + 1
  icase = icase1
