 ...
 for adrec in adrecs:
  ...
  for ientry,entry in enumerate(adrec.entries):
   ...
   if len(accents) != 0:
    ...
    newmeta = None
    newhead = None
    print('changes_p expects no accent in mw k2',metaline)
    exit(1)
   oldk2 = oldk2s[0]
   m = re.search(r'[/\^]',k2pwg)
   # fails if no accent in k2pwg
   mbeg,mend = m.span()  # k2pwg[mbeg] is the accent
   # if k2pwg and oldk2 are the same at indices 0 through mbeg-1,
   # then insert the accent.
   accent = k2pwg[mbeg]
   if k2pwg[0:mbeg] == oldk2[0:mbeg]:
    newk2 = oldk2[:mbeg]+accent+oldk2[mbeg:]
    newmeta = re.sub(r'<k2>(.*?)<','<k2>'+newk2+'<',metaline)
   elif mend== len(k2pwg):
    newk2 = oldk2 + accent
    newmeta = re.sub(r'<k2>(.*?)<','<k2>'+newk2+'<',metaline)
   else:
...
def unused_make_changes_x(adrecs):
...
def make_changes_x(adrecs):
 ...
 for adrec in adrecs:
  ...
  for ientry,entry in enumerate(adrec.entries):
   ...
   accents = re.findall(r'[/\^]',oldk2str)
   if len(accents) != 0:
    print('make_changes_c: no mw accent expected',metaline)
    print(comment)
    exit(1)
   newmeta = oldmeta
   if headline.startswith('¦'):
 ...
 for adrec in adrecs:
  ...
  for ientry,entry in enumerate(adrec.entries):
   ...
   if len(accents) != 0:
    ...
    exit(1)
   newmeta = oldmeta
   oldhead = ';'+oldhead  # expect no changes in headline
   newhead = None
   change = Change(oldmeta,newmeta,lnummeta,oldhead,newhead,lnumhead,comment)
   changes.append(change)
  adrec.changes = changes
 ...
 for adrec in adrecs:
  ...
  for change in changes:
   ...
   if change.newhead == None:
    if change.oldhead.startswith(';'):
     oldhead = change.oldhead[1:] # remove initial ;
    else:
     oldhead = change.oldhead
    outarr.append(';%s old %s' %(change.lnumhead,oldhead))
    outrecs.append(outarr)
    continue
...
class ADrec:
 def __init__(self,line):
  line = line.rstrip('\r\n')
  self.line = line
  self.rawstatus,self.k1,self.k2,self.k2pwg,self.pc = line.split('\t')
  self.status = self.rawstatus.strip()
  if self.status not in ['+','x','c','p']:
