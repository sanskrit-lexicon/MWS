...
def init_Lgroups_cdsl(entries):
 ...
 for group in groups:
  ...
  group.k2ab = ', '.join(group.k2homs)  #AB preference
  # new metaline
...
def write_entries_helper_ab(entry):
 outarr = []
 outarr.append(entry.metaline)
 for line in entry.datalines:
  outarr.append(line)
 grouptxt = entry.grouptxt
 infotxt = entry.infotxt
 lend = '<LEND>%s%s' % (grouptxt,infotxt)
 outarr.append(lend)
 if False and (entry.metad['L'] == '5875.4'):
  print()
  print('write_entries_ab_helper')
  print('metaline:',entry.metaline)
  print('last line:', entry.datalines[-1])
  print('infotxt:',infotxt)
  print('grouptxt:',grouptxt)
  print('lend:',lend)
 return outarr

def write_entries_ab(fileout,entries):
 outrecs = []
 for entry in entries:
  outrec = write_entries_helper_ab(entry)
  outrecs.append(outrec)
 write_recs(fileout,outrecs,blankflag=False)
 ...
 for Lgroup in Lgroups:
  for i,L in enumerate(Lgroup.Ls):
   ...
   if i != 0:
    ...
    continue
   # revise metaline for first entry in group
   entry.metaline = Lgroup.newmeta
   # Ls = ','.join(Lgroup.Ls)
   Ls = ', '.join(Lgroup.Ls)  # AB prefernce
   datalines = entry.datalines
   lastline = datalines[-1]
   Ls1 = '{{%s}}' % Ls

   entry.grouptxt = Ls1
   # ab2a revision. If there are
...
def revise_entries_ab_infotxt(entries):
 for entry in entries:
  # remove <info .*$ from last line of datalines
  # ans save infotxt value in new entry attribute
  datalines = entry.datalines
  lastline = datalines[-1]
  # all <info tags assumed at end of last line
  m1 = re.search(r'(<info .*)$',lastline)
  entry.grouptxt = ''
  if m1 == None:
   entry.infotxt = ''
  else:
   infotxt = m1.group(1)
   newlastline = lastline.replace(infotxt,'')
   datalines[-1] = newlastline
   entry.infotxt = infotxt
  if False and (entry.metad['L'] == '5875.4'):
   print('revise_entries_cdsl_infotxt')
   print('entry.infotxt=',entry.infotxt)
   print('lastline=',lastline)
   print('newlast =',newlastline)
   print(entry.datalines[-1])
         
def revise_entries_cdsl_infotxt(entries):
 for entry in entries:
  entry.lenddata = ''
  # lend may have infotxt.  
  lend = entry.lend
  if lend == '<LEND>':
   continue
  lenddata = lend.replace('<LEND>','')
  if False: #dbg
   if '{{' in lenddata:
    print(lenddata,entry.metad['L'])
    exit(1)
  # put lenddata inty entry.lenddata
  entry.lenddata = lenddata
  # standard value for entry.lend
  entry.lend = '<LEND>'
  # remove {{X}} from lenddata. Result is infotxt
  # append infotxt to last dataline
  infotxt = re.sub(r'{{.*?}}','',lenddata)
  datalines = entry.datalines
  lastline = datalines[-1]
  newlastline = lastline + infotxt
  datalines[-1] = newlastline
if __name__=="__main__":
 ...
 if option == 'cdsl,ab':
  ...
  # Add infotxt attribute and remove from last line
  revise_entries_ab_infotxt(entries)
  entries_ab = make_entries_ab(entries,Lgroups)
  write_entries_ab(fileout,entries_ab)
  print(len(entries_ab),'new entries written to',fileout)
  exit(1)
 if option == 'ab,cdsl':
  revise_entries_cdsl_infotxt(entries)
