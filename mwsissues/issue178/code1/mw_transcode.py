...
import transcoder
# for andhrabharati
transcoder.transcoder_set_dir('transcoder_ab')

slp1chars = {}
#print('skipping update_slp1chars')
...
def update_slp1chars(x,y,tranin,tranout):
 ...
 m = re.search(regex,y)  # 09-02-2024
 if m == None:
  print('Unexpected character in line #%s' % (iline+1,))
  print(' x=',x)
  print(' y=',y)
  for ic,c in enumerate(y):
   if not re.search(regex,c):
    print('%s %s  = char# %s not expected' % (ic+1,c,ord(c)))
   else:
    #print('%s %s  ok' % (ic+1,c))
    pass
