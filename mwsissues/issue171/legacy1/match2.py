...
def check1_entries(entries,k1dict,forcerecs):
 probs = [] # no match 
 for entry in entries:  # add3
  k1 = entry.metad['k1']
  forcerec = get_forcematch_k1(k1,forcerecs,'sup')
  if forcerec != None:
   # no problem
   continue
  if k1 not in k1dict:
...
class Forcematch:
 def __init__(self,line):
  try:
   m = re.search(r'^mw,(.*?),(.*?) sup,(.*?),(.*?)$',line)
   self.mw_L = m.group(1)
   self.mw_k1 = m.group(2)
   self.sup_L = m.group(3)
   self.sup_k1 = m.group(4)
  except:
   print('Forcematch: data error\n',line)
   exit(1)
def init_forcematch(filein):
...
def get_forcematch_k1(k1,recs,src):
 # a linear search is inefficient, but recs is a small list, so
 # inefficiency is small
 for rec in recs:
  if (src == 'mw') and (k1 == rec.mw_k1):
   return rec
  if (src == 'sup') and (k1 == rec.sup_k1):
   return rec
 return None

if __name__=="__main__":
 filein = sys.argv[1] #  xxx.txt (path to digitization of xxx)
 filein1 = sys.argv[2] # add3b.txt
 filein2 = sys.argv[3] # force_match.txt
 fileout = sys.argv[4] # output summary
 entries = digentry.init(filein)
 k1dict = make_k1dict(entries)
 Ldict = digentry.Entry.Ldict;
 print(len(k1dict),'distinct k1 from',filein)
 digentry.Entry.Ldict = {}
 entries1 = digentry.init(filein1)
 forcerecs = init_forcematch(filein2)
 
 problems1 = check1_entries(entries1,k1dict,forcerecs)
