...
for lx,n in top_lex:
    S.append(f'| `{lx}` | {n:,} |')
S.append('\n## Notes')
S.append('- Promoted children span the `<lex>` distribution above (dominated by `n.`,')
S.append('  `ind.` adverbial phrases like `dharmeṇa`, `f.`, `mfn.`, `m.`) — inline derivative')
S.append('  forms MW made separately addressable. A genuine structured-data layer (queryable')
...
if broken_pairs:
    ...
    S.append('- The 31 broken links originally found here (mostly off-by-one L-number typos in')
    S.append('  `phwchild`/`phwparent` targets, plus two `{{Lbody=}}`-alias redirects and one')
    S.append('  duplicate pointer) were corrected in `csl-corrections/batch_pending/dictionaries/mw/`')
    S.append('  (H1500) — this script only analyses/audits, it never mutates `mw.txt`.')
